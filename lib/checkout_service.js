...
service.placeOrder = function(emailParam,callback){
  ...
  _orders_service.createOrder(emailParam, function(statusCode, data){
    if (statusCode==200 && data){
      // Create payment
      service._createChargeAndNotifyToEmail(userEmail,data,function(errCharge,errChargeMsg){
        if (!errCharge){
...
service._createChargeAndNotifyToEmail = function(userEmail,orderInvoice,callback){
  ...
  _stripe.chargeCustomer(orderInvoice.orderNumber, orderInvoice.totalCharge, 'usd', function(errCharge){
    if (!errCharge){
      _orders_service._updateUserInvoicePaid(orderInvoice.orderNumber,function(errUpdatePaid){
...
service._createEmailAndNotifyToEmail = function(orderInvoice,callback){
  ...
  if (emailConfirmation)
  {
    ...
    _mailgun.notifyCustomerByEmail(emailConfirmation, function(errEmail){
      if (!errEmail){
        _orders_service._updateUserInvoiceSent(orderInvoice.orderNumber,function(errUpdateSent){
          if (!errUpdateSent){
            callback(false, false);
          }else {
